...
/**
 * Navbar component for navigation and displaying user info.
 ...
 * @param {string} props.userName - The user's registered username.
 * @param {string} props.dmtBalance - The user's DMT token balance.
 * @param {Array<Object>} props.connectors - Wagmi connectors for displaying connect options.
 * @param {function} props.connect - Wagmi connect function.
 * @param {function} props.disconnect - Wagmi disconnect function.
 * @param {string} props.currentView - The currently active view ('dashboard', 'friends', 'chat').
 * @param {function} props.setCurrentView - Function to set the current view.
 */
function Navbar({
  isConnected,
  address,
  ...
  const navItemClass = (view) =>
    `px-4 py-2 rounded-md transition-colors ${
      currentView === view
        ...
        : 'hover:bg-secondary-dark text-text-light'
    }`;
